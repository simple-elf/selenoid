...
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -tags s3 -ldflags "-X main.buildStamp=`date -u '+%Y-%m-%d_%I:%M:%S%p'` -X main.gitRevision=`git describe --tags || git rev-parse HEAD` -s -w"
